...
describe('User', () => {
  ...
  it('User my apps', (done) => {
    return mockRequest.get('/user/app').set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(200);
      done();
    });
  });

  it('User specific correct app', (done) => {
    return mockRequest.get('/user/app/1').set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(200);
      done();
    });
  });

  it(`User can't get wrong specific app`, (done) => {
    return mockRequest.get('/user/app/19995').set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(500);
      done();
    });
  });

  it('User saved jobs', (done) => {
    return mockRequest.get('/user/saved').set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(200);
      done();
    });
  });

  it('User save job from db', (done) => {
  ...
  it('User save job from api', (done) => {
    return mockRequest.post('/user/save').send({ api:'true', title: 'test', location: 'test', type: 'test', description: 'test', company_name: 'test', phone: 'test', company_url: 'test', logo: 'test', country: 'test' }).set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(201);
      done();
    });
  });

  it(`User can't save job not exist `, (done) => {
    return mockRequest.post('/user/save').send({ jobID:548}).set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(500);
      done();
    });
  });

  it('User delete correct app', (done) => {
    return mockRequest.delete('/user/app/1').set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(202);
      done();
    });
  });

  it(`User can't delete wrong app`, (done) => {
    return mockRequest.delete('/user/app/999').set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(500);
      done();
    });
  });
  ...
  it('User offers', (done) => {
    return mockRequest.get('/user/offers').set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(200);
      done();
    });
  });

  it('User answer correct offer', (done) => {
    return mockRequest.put('/user/offers/1').send({status:'accept'}).set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(201);
      done();
    });
  });

  it(`User can't answer wrong offer`, (done) => {
    return mockRequest.put('/user/offers/99').send({status:'accept'}).set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(500);
      done();
    });
  });

  it(`User edit profile`, (done) => {
    return mockRequest.put('/user/edit').send({first_nameL:'test', last_nameL:'test', phoneL:'test', job_titleL:'test', countryL:'test', ageL:'test', avatarL:'test', experinceL:'test', cvL:'test'}).set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(201);
      done();
    });
  });

  it(`User apply for db job`, (done) => {
    return mockRequest.post('/user/apply/2').send({company_id:3}).set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(201);
      done();
    });
  });

  it(`User apply for api job`, (done) => {
    return mockRequest.post('/user/apply/0').send({api:'true',title:'test', location:'test', type:'test', company_name:'test', logo:'test', email:'test@test.com'}).set('Cookie', [`token=${token}`]).then((result) => {
      expect(result.status).toBe(201);
      done();
    });
  });
