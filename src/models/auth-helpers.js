...
class AuthHelper {
  ...
  async getFullInfo(payload) {
    let SQL = `SELECT *,auth_id,p.id,e.starting_date AS ex_starting_date,e.ending_date AS ex_ending_date,ed.starting_date AS edu_starting_date,ed.ending_date AS edu_ending_date FROM person AS p FULL JOIN experience AS e ON p.id=e.person_id FULL JOIN education AS ed ON p.id=ed.person_id FULL JOIN courses AS c ON p.id=c.person_id WHERE p.auth_id=$1;`;
    let value = [payload];
    let data = await client.query(SQL, value);
    return data.rows;
  }
